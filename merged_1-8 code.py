...
if os.path.exists(file_path):
    ...
    if 'Profit' in df.columns:
        select = st.selectbox("Aggregate by", ["State"])
...
if 'Ship Mode' in df.columns:
    ...
    fig_pie = px.pie(results_df, names='Ship Mode', values=s2, title=f'{s2} Distribution by Ship Mode')
    fig_bar = px.bar(results_df, x='Ship Mode', y=s2, title=f'{s2} by Ship Mode')
    fig_line = px.line(results_df, x='Ship Mode', y=s2, title=f'{s2} by Ship Mode', markers=True)
    fig_scatter = px.scatter(results_df, x='Ship Mode', y=s2, title=f'{s2} by Ship Mode')
...
# Uncomment these lines to display additional plots
# fig_line = px.line(s_df, x=s3, y='Profit', title=f'Top 10 {s32} of Profit per {s3}', markers=True)
# st.plotly_chart(fig_line)
# fig_pie = px.pie(s_df, names=s3, values='Profit', title=f'Top 10 {s32} of Profit per {s3}')
# st.plotly_chart(fig_pie)
...
fig_bar = px.bar(dat45, x='Discount', y=s45, title="Total {} by Discount".format(s45))
st.plotly_chart(fig_bar)

# Uncomment these lines to display additional plots
# fig_line = px.line(dat45, x='Discount', y=s45, title="Total {} by Discount".format(s45))
# st.plotly_chart(fig_line)
# fig_pie = px.pie(dat45, values=s45, names='Discount', title="Total {} by Discount".format(s45))
# st.plotly_chart(fig_pie)
...
if s6 == 'Category':
    fig = px.pie(s_df2, names=level, values=s62, title=f'{s62} by {s6}')
else:
    fig = px.bar(s_df2, x=level, y=s62, title=f'{s62} by {s6}')
...
top_n7 = st.number_input("select top N values", min_value=1, max_value=df[s45].nunique(), value=5, step=1)

summary_df = df.groupby('Customer ID')[s45].sum().reset_index().nlargest(top_n7, s45)
st.write(f"Top {top_n7} {s45} by Customer ID:")
st.write(summary_df)

fig_bar = px.bar(summary_df, x='Customer ID', y=s45, title=f"{s45} by Customer ID")
st.plotly_chart(fig_bar)

# Uncomment these lines to display additional plots
# fig_pie = px.pie(summary_df, names='Customer ID', values=s45, title=f"{s45} by Customer ID")
# st.plotly_chart(fig_pie)
# fig_line = px.line(summary_df, x='Customer ID', y=s45, title=f"{s45} by Customer ID", markers=True)
# st.plotly_chart(fig_line)
...
fig_bar = px.bar(t8_df, x=s8, y='Number of Orders', title=f'Number of Orders by {s8}')
st.plotly_chart(fig_bar)

# Uncomment these lines to display additional plots
# fig_pie = px.pie(t8_df, names=s8, values='Number of Orders', title=f'Number of Orders by {s8}')
# st.plotly_chart(fig_pie)
# fig_line = px.line(t8_df, x=s8, y='Number of Orders', title=f"Number of Orders by {s8}", markers=True)
# st.plotly_chart(fig_line)
